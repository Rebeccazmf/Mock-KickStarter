...
export class BidrecordService {
  ...
  //             .map((res:Response)=>res.json());
  // }
  getIdeaoptions = function(): Observable<Ideaoption[]> {
    return this.http.get(this.url+"/ideaOption/user/"+LoginComponent._userId)
              .map((res:Response)=>res.json());
  }
  getIdeasByStartup = function(/*value:Bidrecord*/): Observable<Idea[]> {
    return this.http.get(this.url+"/idea/startup/"+"4")
    .map((res:Response)=>res.json());
    // return this.http.get(this.url+"/idea/user/"+value.userId,value)
    //           .map((res:Response)=>res.json());
  }
  
  getServices = function(): Observable<Startupservice[]> {
    return this.http.get(this.url+"/service/startup/"+LoginComponent._userId)
              .map((res:Response)=>res.json());
  }
}